...
public class Test17 {
	...
	public static ListNode merge(ListNode head1, ListNode head2) {
		//如果第一个链表为空，返回第二个链表头节点
		if(head1 == null) {
			return head2;
		}
		//如果第二个结点为空，返回第一个链表头节点
		if(head2 == null) {
			return head1;
		}
		//创建一个临时结点，用于添加元素时方便
		...
		//下面的两个if有且只有一个if里面的内容会执行
		//如果第一个链表的元素为处理完将其街道合并链表的最后一个结点之后
		if(head1 != null) {
			pointer.next = head1;
		}
		//如果第二个链表的元素未处理完将其街道合并链表的最后一个节点之后
		if(head2 != null) {
			pointer.next = head2;
		}
	...
	public static ListNode merge2(ListNode head1, ListNode head2) {
		...
		if(tmp.value < head2.value) {
			//如果第一个链表的头结点小，就递归处理第一个链表的下一个节点和第二个链表的头结点
